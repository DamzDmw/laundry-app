...
public class LayananPane {
    ...
    public static Pane create() {
        ...
        grid.add(label("Nama Layanan"), 0, 0); grid.add(tfNama,     1, 0);
        grid.add(label("Jenis"),        0, 1); grid.add(cbJenis,    1, 1);
        grid.add(label("Harga (Rp)"),   0, 2); grid.add(tfHarga,    1, 2);
        grid.add(label("Estimasi Hari"),0, 3); grid.add(tfEstimasi, 1, 3);
        for (int i = 0; i < 4; i++) {
        ...
        btnTambah.setOnAction(e -> {
            if (tfNama.getText().isBlank() || cbJenis.getValue() == null || tfHarga.getText().isBlank()) {
                peringatan("Nama, jenis, dan harga wajib diisi!"); return;
            }
            try {
                ...
                if (dao.insert(l)) { data.setAll(dao.getAll()); bersihkanForm(tfNama, tfHarga, tfEstimasi, cbJenis); }
                else peringatan("Gagal menambah layanan!");
            } catch (NumberFormatException ex) { peringatan("Harga dan estimasi harus berupa angka!"); }
        });

        btnEdit.setOnAction(e -> {
            Layanan pilihan = tabel.getSelectionModel().getSelectedItem();
            if (pilihan == null) { peringatan("Pilih layanan yang ingin diedit!"); return; }
            try {
                ...
                pilihan.setEstimasiHari(tfEstimasi.getText().isBlank() ? 2 : Integer.parseInt(tfEstimasi.getText().trim()));
                if (dao.update(pilihan)) { data.setAll(dao.getAll()); bersihkanForm(tfNama, tfHarga, tfEstimasi, cbJenis); }
                else peringatan("Gagal mengedit layanan!");
            } catch (NumberFormatException ex) { peringatan("Harga dan estimasi harus berupa angka!"); }
        });

        btnHapus.setOnAction(e -> {
            Layanan pilihan = tabel.getSelectionModel().getSelectedItem();
            if (pilihan == null) { peringatan("Pilih layanan yang ingin dihapus!"); return; }
            if (konfirmasi("Hapus layanan " + pilihan.getNamaLayanan() + "?")) {
                if (dao.delete(pilihan.getIdLayanan())) data.setAll(dao.getAll());
        ...
        btnBatal.setOnAction(e -> { tabel.getSelectionModel().clearSelection(); bersihkanForm(tfNama, tfHarga, tfEstimasi, cbJenis); });
    ...
    private static void bersihkanForm(TextField tfNama, TextField tfHarga, TextField tfEst, ComboBox<String> cb) {
        tfNama.clear(); tfHarga.clear(); tfEst.clear(); cb.setValue(null);
    }
    ...
    private static boolean konfirmasi(String pesan) {
        Optional<ButtonType> h = new Alert(Alert.AlertType.CONFIRMATION, pesan, ButtonType.YES, ButtonType.NO).showAndWait();
