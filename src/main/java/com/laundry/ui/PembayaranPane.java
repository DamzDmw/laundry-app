...
public class PembayaranPane {
    ...
    public static Pane create() {
        ...
        for (Transaksi t : tDao.getAll()) {
            if (!dao.sudahDibayar(t.getIdTransaksi()))
                cbTransaksi.getItems().add(t);
        }
        ...
        cbTransaksi.setConverter(new javafx.util.StringConverter<>() {
            ...
            @Override
            public Transaksi fromString(String s) {
                return null;
            }
        });
        ...
        grid.setHgap(16);
        grid.setVgap(10);
        grid.add(label("Transaksi"), 0, 0);
        grid.add(cbTransaksi, 1, 0);
        grid.add(label("Total Bayar"), 0, 1);
        grid.add(tfTotal, 1, 1);
        grid.add(label("Diskon (Rp)"), 0, 2);
        grid.add(tfDiskon, 1, 2);
        grid.add(label("Metode"), 0, 3);
        grid.add(cbMetode, 1, 3);
        for (int i = 0; i < 4; i++)
        ...
        btnSimpan.setStyle(StyleHelper.btnHijau());
        ...
        btnSimpan.setOnAction(e -> {
            if (cbTransaksi.getValue() == null || tfTotal.getText().isBlank() || cbMetode.getValue() == null) {
                peringatan("Transaksi, total bayar, dan metode wajib diisi!");
                return;
            }
            try {
                // Tambahan: validasi total bayar harus lebih dari 0
                double totalBayar = Double.parseDouble(tfTotal.getText().trim());
                if (totalBayar <= 0) {
                    peringatan("Total bayar harus lebih dari 0!");
                    return;
                }

                Pembayaran pb = new Pembayaran();
                pb.setIdTransaksi(cbTransaksi.getValue().getIdTransaksi());
                pb.setTotalBayar(totalBayar);
                ...
                if (dao.insert(pb)) {
                    ...
                    peringatan("Gagal menyimpan pembayaran!");
            } catch (NumberFormatException ex) {
                peringatan("Total bayar dan diskon harus berupa angka!");
            }
        });
